...
If the file exists, check `cached_at` — if less than **24 hours** old, use cached values directly and continue to Step 2. Docker daemon checks are intentionally deferred until the workflow actually needs to build an image.
...
# Build-only; do not block preflight if missing
...
### 1.3 Docker Availability (deferred warning only)

Docker is **not** an entry blocker. The deploy path may reuse an existing Docker Hub/GHCR/README/Compose image and skip local build entirely, so missing Docker must not stop Phase 1 or Phase 2.

If `docker --version` failed, record:
```
ENV.docker = false
ENV.docker_deferred = true
```
Report it as a warning only:
> Docker CLI is unavailable. This is OK if Phase 2 finds a reusable image; Docker will only be required later if the pipeline must build and push a new image.

Do **not** ask to install Docker here. Do **not** run `docker info` here. Docker daemon checks belong to Phase 4 Build & Push only.
...
Environment:                      (cached / refreshed)
  ○ Docker <version or "missing — deferred until build is required">
  ✓ git <version>
...
Note: Docker CLI/daemon and Docker Hub login are NOT checked here. They are only required if Phase 2 finds no existing image and we need to build & push (Phase 4).
