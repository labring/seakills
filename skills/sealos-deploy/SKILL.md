...
compatibility: Requires git and Sealos auth. Docker is required only when no reusable image is found and the workflow must build/push locally. Optional Node.js 18+, Python 3.8+.
metadata:
  author: zjy365
...
Legacy `/sealos-deploy` remains the default path. Workflow mode never silently falls back to the legacy path; if workflow recovery fails, stop and surface the recovery command.
...
[2026-03-05 14:30:01] Docker: ○ missing — deferred until build is required
...
| `sealos-auth.mjs` | `node sealos-auth.mjs check\|login\|begin-login\|complete-login\|list\|switch` | Sealos Cloud authentication |
...
|-------|--------|-----------|
| 0 — Preflight | git/curl/jq + Sealos auth; Docker warning only | Entry blockers resolved |
